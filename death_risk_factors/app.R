             ...
             sidebarLayout(
               ...
               mainPanel(fluidPage(
                 ...
                 h4("As measured by the number of deaths per 100,000 people",
                    align = "left"),
                 bsTooltip(id = "plot_substance", title = "The figure is a world map visualization titled \\'Death Rate of Selected Substance Type by Country\\' that displays the death rate, as measured by the number of deaths per 100,000 people, due to the selected type of substance issue. The four possible types of substances that can be selected are \\'Secondhand Smoking,\\' \\'Alcohol Use,\\' \\'Drug Use,\\' and \\'Smoking.\\' Each country on the map is assigned a color on a gradient scale based on the value of the death rate. The user can view the exact death rate for a specific country by hovering over the country of interest. Moreover, the user can use a timeline slider below the map to select a year of interest in order to see what the death rate is in that year. The range of years is is every three years from 1990 to 2015. For more details on specific trends observed from the map over time, please see the analysis section of the write up.",
                           placement = "bottom",
                           trigger = "hover",
                           options = NULL),
    ...
    tabPanel("Sanitation",
             sidebarLayout(
               ...
               mainPanel(fluidPage(
                 ...
                 h4("As measured by the number of deaths per 100,000 people",
                    align = "left"),
                 bsTooltip(id = "plot_sanitation", title = "The figure is a world map visualization titled \\'Death Rate of Selected Sanitation Issue by Country\\' that displays the death rate, as measured by the number of deaths per 100,000 people, due to the selected sanitation issue. The three possible sanitation issues that can be selected are \\'Unsafe Water Source,\\' \\'Unsafe Sanitation,\\' and \\'No Hand Wash.\\' Each country on the map is assigned a color on a gradient scale based on the value of the death rate. The user can view the exact death rate for a specific country by hovering over the country of interest. Moreover, the user can use a timeline slider below the map to select a year of interest in order to see what the death rate is in that year. The range of years is is every three years from 1990 to 2015. For more details on specific trends observed from the map over time, please see the analysis section of the write up.",
                           placement = "bottom",
                           trigger = "hover",
                           options = NULL),
