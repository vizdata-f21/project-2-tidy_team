---
title: "Global Deaths by Risk Factor"
subtitle: "A shiny app hosting interactive spatio-temporal visualizations. "
author: "Tidy Team  <br> Drew Flanagan, Courtney Lee, Kathryn Thomas, Raghav Rasal"
institute: "Duke University"
date: "`r Sys.Date()`"
output:
  xaringan::moon_reader:
    css: xaringan-themer.css
    lib_dir: libs
    nature:
      ratio: "16:9"
      highlightStyle: github
      highlightLines: true
      countIncrementalSlides: false
---

```{r load-packages, include = FALSE}
# Add any additional packages you need to this chunk
# Remove any packages from this list that you're not using
library(tidyverse)
library(knitr)
library(xaringanthemer)
library(here)
```

```{r setup, include = FALSE}
# For better figure resolution
knitr::opts_chunk$set(
  fig.retina = 3, 
  dpi = 300, 
  fig.width = 6, 
  fig.asp = 0.618, 
  out.width = "70%"
  )
```

```{r title-slide, echo = FALSE}
style_xaringan(title_slide_background_image = 
                 here("presentation","presentation_files","title_image.jpg"))
```

class: inverse

# Motivation 

.pull-left[

- Aim and motivation: Communicate the harmful effects of certain risk factors on 
the world through time via a Shiny app

- Potential Users:

  - Global researchers, policymakers, citizens

- 3 Categories of Risk Factors: 

1. air pollution

2. substance use

3. sanitation
  
]

```{r risk-factor, echo = FALSE, out.width = "50%", fig.align = "right"}
include_graphics(here("presentation","presentation_files","airpollution.jpg"))
```

---

class: inverse, middle

## Approach

.pull-left[

- Data cleaning and merging
  
  - Utilizing Rds files
  
- 3 main data sets:

  - The Death by Risk Factor Data from Our World in Data via Kaggle 
  
  - The World Map Data from Maps Package
  
  - Population Data from Our World in Data
  
  - The World Map Data from Maps Package
  
- Map with a time and interactive functionality

  - Plotly

- Line plots 

]

```{r approach-image, echo = FALSE, out.width = "50%", fig.align = "right"}
include_graphics(here("presentation","presentation_files","coding.jpg"))
```

---

### Map Example

```{r map-example, echo = FALSE, out.width = "60%", fig.align = "center"}
include_graphics(here("presentation","presentation_files","map.jpg"))
```

---

### Line Graph Example

```{r line-graph-example, echo = FALSE, out.width = "90%", fig.align = "center"}
include_graphics(here("presentation","presentation_files","linegraph.jpg"))
```

---

class: inverse

# The Shiny App

<iframe height="360" width="640" src="https://warpwire.duke.edu/w/-VUGAA/" frameborder="0" scrolling="0" allow="autoplay *; encrypted-media *; fullscreen *; picture-in-picture *;" allowfullscreen></iframe>

---

class: inverse, middle

## Trends

- Death rates attributable to air pollution in total have consistently fallen 
throughout the 25 years on the time slider. For instance, the US’ death rate due 
to air pollution (total) has dropped from 31.19 in 1990 to 19.95 in 2015. 

- China and Russia faced the highest death rates due to secondhand smoking, with 
the latter experiencing continuously rising death rates until 2008, after which 
death rates started to plateau. 

- The region that has the highest death rates due to alcohol is Eastern Europe, 
particularly Russia, Romania and Ukraine, each having a death rate higher than 
2.

- The regions with the highest death rates due to unsafe water sources are 
Africa, South America and South/South East Asia. 


```{r risk-factor-1990_air, echo = FALSE, out.width = "50%", fig.align = "right"}
  include_graphics(here("presentation","presentation_files","1990_air.PNG"))
```

```{r risk-factor-2015_air, echo = FALSE, out.width = "50%", fig.align = "right"}
  include_graphics(here("presentation","presentation_files","air_2015.PNG"))
```
---

class: inverse, middle

## Limitations

- Speed of Shiny app

- Not all age standardized

- Variable names as labels

- Did not include all risk categories



---

class: inverse, middle, center

# Thank you for listening!



